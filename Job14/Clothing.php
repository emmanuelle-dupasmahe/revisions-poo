...
class Clothing extends AbstractProduct implements StockableInterface {
    ...
    public function addStocks(int $stock): self {
        if ($stock > 0) {
            // $this->quantity est protected dans AbstractProduct, donc accessible ici.
            $current_quantity = $this->getQuantity() ?? 0;
            $this->setQuantity($current_quantity + $stock);
        }
        return $this;
    }
    ...
    public function removeStocks(int $stock): self {
        if ($stock > 0) {
            $current_quantity = $this->getQuantity() ?? 0;
            $new_quantity = max(0, $current_quantity - $stock); // Empêche une quantité négative
